...
class CompleteConceptInfo extends Command
{
    ...
    public function handle(
        ...
        if (is_string($localeRaw) && trim($localeRaw) !== '') {
            $localeCandidate = strtolower(trim($localeRaw));
            $supported = ConceptLocale::supported();
            if (! in_array($localeCandidate, $supported, true)
                && ! in_array(ConceptLocale::resolve($localeCandidate), $supported, true)) {
                $this->error('Locale must be in supported list: '.implode(', ', $supported));

                return self::FAILURE;
            }
            $locale = ConceptLocale::resolve($localeCandidate);
        }
