...
class ConceptCompleteInfoDispatchService
{
    ...
    public function dispatch(
        ...
        if ($locale !== null && trim($locale) !== '') {
            $resolvedLocale = ConceptLocale::resolve($locale);
            if (! in_array($resolvedLocale, ConceptLocale::supported(), true)) {
                throw new InvalidArgumentException(
                    'Locale must be in supported list: '.implode(', ', ConceptLocale::supported())
                );
            }
        }
